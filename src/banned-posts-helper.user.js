...
const HandlePostShowPage = {
  ...
  patchSidebarFav(isFavorited, optionSection) {
    optionSection.querySelector(isFavorited ? "#remove-from-favorites" : "#add-to-favorites").style.display = "unset";
  },
  ...
  patchEditForm({ source, rating, tag_string_artist, tag_string_copyright, tag_string_character, tag_string_meta, tag_string_general, media_asset }) {
    const ratingHtml = r =>
      `<input class="radio_buttons required" required="required" type="radio" value="${r}" name="post[rating]" id="post_rating_${r}"${r === rating ? " checked" : ""}>`;
  ...
  async initialize() {
    ...
    const [, , , , postNotice, postRelationshipContainers] = await Promise.all([
      ...
      this.patchSidebarFav(isFavorited, optionSection),
