...
const TooltipHelper = {
  ...
  globalTooltipConfig: [
    {
      ...
      options: {
        ...
        templateEngineFunc: content => {
          ...
          let p = `<p><a target="_blank" href="/artists/${uid}/edit">Edit</a>`;
          if (uid === "null") p = `<p><a target="_blank" href="/artists/new?artist%5Bname%5D=${name}">Create New</a>`;
          p += `&nbsp;|&nbsp;<a target="_blank" href="/post_versions?search%5Bchanged_tags%5D=${name}">Post Changes</a>&nbsp;|&nbsp;<a target="_blank" href="/reports/posts?search%5Bfrom%5D=2005-05-23&search%5Bgroup%5D=uploader&search%5Bgroup_limit%5D=200&search%5Bmode%5D=table&search%5Btags%5D=${name}">Uploaders</a><span>`;
    ...
    },
    {
      ...
      options: {
        ...
        templateEngineFunc: (content, instance) => {
          ...
          file_size = Booru.numberToHumanSize(file_size);
          duration = duration ? ` (${Booru.secondsToClock(duration)})` : "";
  ...
  bindTooltipsToElements(container = document) {
    ...
    Booru.isMobile && this.clickFixer(container);
  },
  hookDtextPreview() {
        ...
        if (Array.isArray(json)) {
          ...
          $("li#post-info-favgroups > a").stt({
            ...
            templateEngineFunc: (content, instance) => {
              ...
              let html = '<table class="stt-table stt-favgroup-table text-xs"><thead><tr><th>Group</th><th>User</th></tr></thead><tbody>';
              for (let {
                id,
                name,
          ...
          Booru.isMobile && this.clickFixer("li#post-info-favgroups");
        }
      });
  },
  clickFixer(container, selector = ".post-score>a,.post-favcount>a,a.stt-style") {
    $(container)
      .find(selector)
      .on("click", function (event) {
        ...
        if (!$el.data("focused")) {
          ...
          $el.data("focused", "1");
          $el.one("blur", () => $el.removeData("focused"));
        }
      });
  }
};
...
const TagDiffView = (() => {
  ...
  TDV.loadAllVersions = async function () {
    ...
    try {
      ...
      TagListManager.insertTagElements(this.tagList, this.TAG_CACHE, false, [...this.TAG_REMOVED], li =>
        li.classList.add("tag-diff-view", "tag-diff-view-removed", "tag-diff-view-hidden")
      );
      this.completed = true;
    } catch (e) {
      console.warn(e);
  ...
  TDV.fillTagCache = async function () {
    ...
    if (tags.size) await TagListManager.fetchTagData(tags, this.TAG_CACHE);
  };
...
if (dataset.action !== "error" && dataset.controller !== "static") {
  TooltipHelper.initialize();
  if (dataset.action === "show" && dataset.controller === "posts") {
    TooltipHelper.initFavgroupCountTooltip();
    TagDiffView.initialize();
  }
} else
  Booru.BPHLoaded(() => {
    if (document.title.startsWith("Page Removed") && (location.pathname === "/posts" || location.pathname === "/posts.html")) TooltipHelper.initialize();
    else if (dataset.action === "show" && dataset.controller === "posts") {
      TooltipHelper.initialize();
      TooltipHelper.initFavgroupCountTooltip();
      TagDiffView.initialize();
    }
